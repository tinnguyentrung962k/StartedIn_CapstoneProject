...
namespace StartedIn.Service.Services
{
    public class DisbursementService : IDisbursementService
    {
        ...
        public async Task<InvestorDisbursementOverviewDTO> GetInvestorDisbursementOverviewAsync(string userId)
        {
            // Define date ranges
            ...
            var overall = await GetDisbursementOverview(userId, null, null);

            // Calculate last month totals
            var lastMonth = await GetDisbursementOverview(userId, startOfLastMonth, endOfLastMonth);

            // Calculate current month totals
            var currentMonth = await GetDisbursementOverview(userId, startOfCurrentMonth, endOfCurrentMonth);

            // Calculate next month totals
            var nextMonth = await GetDisbursementOverview(userId, startOfNextMonth, endOfNextMonth);
        ...
        private async Task<DisbursementOverviewItem> GetDisbursementOverview(string userId, DateOnly? startDate, DateOnly? endDate)
        {
            decimal totalDisbursementAmount = 0;
            decimal disbursedAmount = 0;
            decimal notDisbursedAmount = 0;
            ...
            if (startDate.HasValue && endDate.HasValue)
            {
                ...
                var totalDisbursement = await _disbursementRepository.QueryHelper()
                 ...
                 && x.EndDate >= startDate && x.EndDate <= endDate).GetAllAsync();
                totalDisbursementAmount = totalDisbursement.Sum(x => x.Amount);
                ...
                disbursedAmount = disbursed.Sum(x => x.Amount);

                // Not disbursed amount
                var notDisbursed = await _disbursementRepository.QueryHelper()
                    .Filter(x => x.InvestorId.Equals(userId)
                    && x.IsValidWithContract == true
                    && x.EndDate >= startDate && x.EndDate <= endDate
                    && x.DisbursementStatus != DisbursementStatusEnum.FINISHED).GetAllAsync();
                notDisbursedAmount = notDisbursed.Sum(x => x.Amount);
            }
            else 
            {
                var totalDisbursement = await _disbursementRepository.QueryHelper()
        ...
        public async Task SetAllDisbursementToOngoing()
        {
            var disbursements = await _disbursementRepository.QueryHelper()
                .Filter(d => d.DisbursementStatus == DisbursementStatusEnum.PENDING && d.IsValidWithContract == true).GetAllAsync();
            foreach (var disbursement in disbursements)
            {
                disbursement.DisbursementStatus = DisbursementStatusEnum.ONGOING;
                _disbursementRepository.Update(disbursement);
            }
            await _unitOfWork.SaveChangesAsync();
        }

        public async Task SetDisbursementStatusToOngoing()
        {
            var disbursements = await _disbursementRepository.QueryHelper()
                .Filter(d => d.DisbursementStatus == DisbursementStatusEnum.PENDING && d.IsValidWithContract == true
                             && d.StartDate == DateOnly.FromDateTime(DateTime.Today))
                .Include(d => d.Investor)
            ...
            foreach (var disbursement in disbursements)
            {
                ...
                if (investor != null && project != null)
                {
                    ...
                    {
                        // Gửi email nhắc nhở cho nhà đầu tư
                        await _emailService.SendDisbursementStartReminder(
                            investor.Email,
                    ...
                    {
                        _logger.LogError($"Error sending reminder email to {investor.Email} for disbursement {disbursement.Title}: {ex.Message}");
                    }
                }
                else
                {
                    _logger.LogWarning($"Investor or Project not found for Disbursement ID: {disbursement.Id}");
                }
